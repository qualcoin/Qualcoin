...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizequalcoinAmount"))
        settings.setValue("nAnonymizequalcoinAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizequalcoinAmount = settings.value("nAnonymizequalcoinAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizequalcoinAmount"))
        SoftSetArg("-anonymizequalcoinamount", settings.value("nAnonymizequalcoinAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizequalcoinAmount:
            nAnonymizequalcoinAmount = value.toInt();
            settings.setValue("nAnonymizequalcoinAmount", nAnonymizequalcoinAmount);
            emit anonymizequalcoinAmountChanged(nAnonymizequalcoinAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
