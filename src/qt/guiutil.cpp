...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("qualcoin"))
        return false;
...
{
    // Convert qualcoin:// to qualcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("qualcoin://"))
    {
        uri.replace(0, 12, "qualcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "qualcoin.lnk";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart)
    {
        ...
        // Get a pointer to the IqualcoinLink interface.
        IShellLink* psl = NULL;
        ...
        if (SUCCEEDED(hres))
        {
            ...
            // Query IqualcoinLink for the IPersistFile interface for
...
{
    return GetAutostartDir() / "qualcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("qualcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  qualcoin-qt [" + tr("command-line options") + "]                     " + "\n";
