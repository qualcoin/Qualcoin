...
unsigned int nStakeMaxAge = 10 * 24 * 60 * 60; // 10 Days
unsigned int nModifierInterval = 9 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 3; // 4 in total
...
const string strMessageMagic = "qualcoin Signed Message:\n";
...
{
    int64_t nSubsidy = nCoinAge * COIN_YEAR_REWARD * 33 / (365 * 33 + 8);  // 50% yearly interest 
    
    return nSubsidy + nFees;
}

static int64_t nTargetTimespan = 9 * 60;  // 9 mins
