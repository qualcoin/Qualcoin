...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #qualcoincoinTEST\r");
            Send(hSocket, "WHO #qualcoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #qualcoincoin\n");
            // randomly join #qualcoincoin00-#qualcoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #qualcoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #qualcoincoin%02d\r", channel_number).c_str());
        };
